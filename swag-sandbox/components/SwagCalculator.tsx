...
import type { SwagSpec, BrandProfile, BenefitType, BenefitResult, Department, Category } from '@/lib/swag-types'
import { DEFAULT_BRAND_PROFILE, BENEFIT_TYPE_META, DEPARTMENTS, CATEGORIES, CATEGORY_OVERRIDES } from '@/lib/swag-types'
import CustomDropdown from './CustomDropdown'
import { computeSwag, getRequiredBrandInputs, buildTemplateVars, resolveNarrative } from '@/lib/swag-engine'
import { highlightGreen } from '@/lib/highlight'
import { fmtMoney, fmtMoneyCompact, fmtNumber, fmtMultiple } from '@/lib/format'
    ...
    <>
      ...
      <main className="min-h-screen px-6 pt-20 pb-10 max-w-[1400px] mx-auto">
        ...
        {ctaFixed && (
          <button
            onClick={() => setShowIntroModal(true)}
            className="group fixed top-16 z-40 btn-primary px-5 py-2 text-sm flex items-center gap-2 shadow-lg shadow-accent-green/20 print:hidden"
            style={{ right: ctaRight }}
          >
            Ask for an intro
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="arrow-nudge">
              <path d="M5 12h14M12 5l7 7-7 7" />
